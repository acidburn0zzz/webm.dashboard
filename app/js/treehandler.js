    ...
    if ((tagName == "A" || tagName == "INS") &&
      ...
      if (state1.length > 0){
        ...
        if (state2.length == 0) {
          ...
          // We bind the change_state handler each time the tree is created
          $("#treeView2").bind("change_state.jstree", function (e, d) {
            ...
            if ((tagName == "A" || tagName == "INS") &&
              ...
              // We add or remove the next tree as necessary
              if (state2.length > 0){
                $("#treeView3").jstree(treeConfig);
              }
              else { // We need to destroy the next tree (if exists)
                $("#treeView3").empty();
              }
            }
          });
